      ...
      <div className="max-w-7xl mx-auto px-6 space-y-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((p, i) => (
            <Card
              key={i}
              className={`rounded-3xl border border-divider bg-surface p-8 shadow-sm space-y-5 ${p.disabled ? "opacity-60" : ""}`}
            >
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-text-primary">{p.name}</h3>
                {p.name === "Creator" && <Badge variant="neutral">Popular • Soon</Badge>}
                {p.name === "Studio" && <Badge variant="neutral">Coming Soon</Badge>}
              </div>
              ...
              {p.cta ? (
                <Button variant="primary" className="w-full font-bold" disabled={p.disabled}>
                  {p.cta}
                </Button>
              ) : null}
            </Card>
